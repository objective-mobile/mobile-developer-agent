...
def _build_agent():
    ...
    return create_react_agent(model=llm, tools=[web_search, knowledge_search], prompt=PLANNER_PROMPT)
...
def run_planner(request: str) -> ResearchPlan:
    ...
    if m:
        try:
            return ResearchPlan(**json.loads(m.group(1)))
        except Exception:
            pass
    try:
        return ResearchPlan(**json.loads(text))
    except Exception:
        pass
    ...
    return ResearchPlan(
        ...
        output_format="structured markdown report with sections and summary",
    )
